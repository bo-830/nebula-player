...
function mergeWithDefaults(defaults, stored, added) {
  ...
  for (const key of Object.keys(defaults)) {
    ...
    out[key] = mergeWithDefaults(defaults[key], stored[key], added);
  }
...
const SUPPORTED_EXTS = /* @__PURE__ */ new Set([".mp3", ".wav", ".flac", ".aac", ".m4a", ".ape", ".ogg", ".opus"]);
...
class LibraryService {
   ...
   */
  async scanRoots(roots2, onProgress, cancelled) {
...
function uniqueToolId(seen, candidate) {
  let id = candidate;
  while (seen.has(id)) id = `${FALLBACK_TOOL_CALL_PREFIX}${++fallbackToolCallSeq}`;
  seen.add(id);
  return id;
}
function withUniqueToolIds(calls) {
  const seen = /* @__PURE__ */ new Set();
  return calls.map((tc) => {
    const id = tc.id ? uniqueToolId(seen, tc.id) : `${FALLBACK_TOOL_CALL_PREFIX}${++fallbackToolCallSeq}`;
    return id === tc.id ? tc : { ...tc, id };
  });
}
async function chatComplete(config, messages, tools, emitChunk, abort) {
  const url2 = normalizeBaseURL(config.baseURL);
  const body = {
    model: config.model,
    messages,
    stream: true
...
async function testApi(config) {
  ...
  try {
    const url2 = base.endsWith("/models") ? base : base + "/models";
    try {
...
function toAbsolute(roots2) {
  return roots2.filter((r) => typeof r === "string" && r.length > 0).map((r) => path.resolve(r));
}
function createMediaRoots(options) {
  let allowed = [];
  let pending = null;
  const populate = async () => {
    const roots2 = new Set(toAbsolute(options.staticRoots()));
    try {
      const parsed = await options.readLibrary();
      const tracks = parsed;
      for (const t of tracks) {
        if (typeof t.path === "string" && t.path) roots2.add(path.resolve(path.dirname(t.path)));
        if (typeof t.coverPath === "string" && t.coverPath) roots2.add(path.resolve(path.dirname(t.coverPath)));
      }
    } catch {
    }
    return Array.from(roots2);
  };
  const get = async () => {
    if (pending) return pending;
    const promise = populate().then((roots2) => {
      if (pending === promise) allowed = roots2;
      return allowed;
    });
    pending = promise;
    return promise;
...
const roots = createMediaRoots({
  ...
  readLibrary: async () => JSON.parse(await fs.promises.readFile(path.join(paths().userData, "library.json"), "utf-8"))
});
...
function registerMediaProtocol() {
  electron.protocol.handle("media", async (request) => {
    ...
    if (!SERVABLE_EXTS.has(path.extname(filePath).toLowerCase())) {
...
function needsConvert(path2) {
  const idx = path2.lastIndexOf(".");
  const ext = idx >= 0 ? path2.slice(idx).toLowerCase() : "";
...
function cacheFileFor(mediaPath, targetExt) {
  return path.join(
    paths().decodeCache,
    `${crypto.createHash("sha1").update(mediaPath.toLowerCase()).digest("hex")}.${targetExt}`
  );
}
...
async function ensurePlayable(mediaPath) {
  ...
  const bin = ffmpegBin();
  const args = targetExt === "m4a" ? ["-y", "-i", mediaPath, "-c", "copy", "-movflags", "+faststart", "-f", "mp4", tmp] : ["-y", "-i", mediaPath, "-vn", "-c:a", "pcm_s16le", "-f", "wav", tmp];
  try {
...
function feedFor(url2) {
  return { provider: "generic", url: url2.endsWith("/") ? url2 : url2 + "/" };
}
async function checkUpdate(feedURL) {
  setState({ state: "checking" });
  const url2 = feedURL?.trim();
  if (!url2) {
  ...
  try {
    if (!electron.app.isPackaged) {
      electronUpdater.autoUpdater.setFeedURL(feedFor(url2));
    } else {
      try {
        electronUpdater.autoUpdater.setFeedURL(feedFor(url2));
      } catch {
...
const MINI_COLLAPSED = { width: 360, height: 128 };
const MINI_EXPANDED = { width: 360, height: 540 };
function computeMiniBounds(current2, expanded, workArea) {
  ...
  return {
    x: Math.min(Math.max(current2.x, workArea.x), maxX),
    y: Math.min(Math.max(current2.y, workArea.y), maxY),
    width,
    height
  };
}
...
function setMiniExpanded(expanded) {
  ...
  if (!w) return { ...nominal };
  const current2 = w.getBounds();
  const workArea = electron.screen.getDisplayMatching(current2).workArea;
  const next = computeMiniBounds(current2, expanded, workArea);
  w.setBounds(next);
...
function registerIpc(svc) {
  ...
  electron.ipcMain.handle("library:scan", async (e, roots2) => {
  ...
  electron.ipcMain.handle("mini:expand", (_e, v) => setMiniExpanded(!!v));
  electron.ipcMain.on("mini:play", (_e, p) => svc.win()?.send("mini:play", p));
  electron.ipcMain.on("chat:proxy:cmd", (_e, p) => svc.win()?.send("chat:proxy:cmd", p));
  electron.ipcMain.on("chat:proxy:state", (_e, s) => forwardToMini("chat:proxy:state", s));
...
const DEV_ORIGINS = /* @__PURE__ */ new Set([
  "http://localhost:5173",
  "http://127.0.0.1:5173",
  "http://localhost:5174",
  "http://127.0.0.1:5174"
]);
const RENDERER_URL_PREFIX = url.pathToFileURL(path.join(__dirname, "..", "renderer") + path.sep).pathname;
function isAppPage(rawUrl) {
  ...
  if (utils.is.dev) return DEV_ORIGINS.has(url2.origin);
  return url2.protocol === "file:" && url2.pathname.startsWith(RENDERER_URL_PREFIX);
}
function externalLinkTarget(rawUrl) {
  let url2;
  try {
    url2 = new URL(rawUrl);
  } catch {
  ...
  return url2.protocol === "http:" || url2.protocol === "https:" ? url2.toString() : null;
}
function guardWindowOpen(contents) {
  contents.setWindowOpenHandler(({ url: url2 }) => {
    const external = externalLinkTarget(url2);
    if (external) {
      void electron.shell.openExternal(external).catch((err) => {
...
function guardNavigation(contents) {
  const guard = (event, url2) => {
    if (isAppPage(url2)) return;
    event.preventDefault();
    log("warn", `[security] blocked navigation to ${url2}`);
  };
