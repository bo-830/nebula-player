...
| A-6 | **口吻分档目前纯提示词委托、应用侧零强制**（t56 §③ 裁定：可接受、不阻塞） | 安全文案（确认条/chip/按钮/失败指路）**全部代码自产** ⇒ 模型过度卖萌也无法削弱确认语义；最坏只是风格瑕疵 | **推荐加固**：只在**代码已知的破坏性回合**（`result.needsConfirm` → `PendingConfirm.finalText`）做确定性 emoji/卖萌词剥离；其余档位保持提示词约束 |
...
| B-9 | **助手名「薇拉 Vela」有 3 处字面量**（`tools.ts` 的 `ASSISTANT_NAME`、`ChatPanel.tsx`、`MiniChat.tsx` 各一个 `ASSISTANT_TITLE`） | reviewer t56 §④ 指出：`react-refresh` 规则并不禁止新增一个**纯 `.ts` 常量模块**（如 `lib/assistantName.ts`）让三处 import ⇒ 可做到真正单一来源（非必须，现有测试已能"漂移必红"） |
| B-10 | **t55 未取证项**：模型侧口吻遵从、打包态、迷你窗确认条 DOM、t54 时代 test 哈希、MiniChat「改前无标题」对比 | 其中打包态与迷你窗确认条 DOM 建议由发布波次的打包态冒烟覆盖 |
| B-11 | **打包态 UI 无法被程序化驱动**（1.0.6 发布时暴露）：打包渲染进程**没有 dev 的 `window.__nebula` 调试面**，CDP 下 DOM 选择器取不到 `.chat-msg.ai`、全文检索不到标题、无 `#mini` 目标 ⇒ 打包态慢滴注与迷你窗形态两项冒烟**未取证**（记取证缺口，非产品缺陷；captain 已用产物级断言补偿：`薇拉 Vela`×3、`stopRevealTimer` 新模式 PRESENT、旧模式 absent、mini-search/mini-chat-head/chat:proxy 均在包内、安装副本 asar = dist asar） | 下一版给打包态加**最小可驱动面**（例如仅当设置 `NEBULA_SMOKE=1` 时暴露只读 DOM 探针或一个 smoke IPC 通道），使打包态冒烟可自动化 |
| B-12 | **进程级死亡观察为部分取证**：1.0.6 只做了存活/进程数（76s、121s 两次存活），**未捕获异常计数** | 接 `crashReporter` + 异常计数采样，或写入应用日志并在冒烟里断言 `uncaughtException=0` |
...
| C-7 | 失败提示的克制风格 | `⚠️` 前缀按裁决**维持现状**（功能性警示图标，非人格装饰）；位置：`chatStore.ts`（t58 改写后为 **`:664`**，t58 前是 `:624`）——**按字符串 `无法获取 AI 回复` 定位更稳**。若要与人格统一（提示词写明「emoji 仅此类回复」），下一版一并处理，同时改掉 t54 里那条"KNOWN GAP"断言 |
