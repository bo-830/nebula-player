...
function Particles() {
  ...
  reactExports.useEffect(() => {
    ...
    const parts = Array.from({ length: N }, () => ({
      ...
      hue: ["cyan", "purple", "blue"][Math.floor(Math.random() * 3)]
    }));
    const tick = (t) => {
...
function TitleBar() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "tb", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tb-logo", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tb-logo-mark" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        "NEBULA ",
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "gradient-text", children: "PLAYER" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tb-titlebar" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tb-controls no-drag", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "tb-btn", title: "最小化", onClick: () => window.api.windowMinimize(), children: /* @__PURE__ */ jsxRuntimeExports.jsx(Minus, { size: 14 }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          children: maximized ? /* @__PURE__ */ jsxRuntimeExports.jsx(Copy, { size: 13 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Square, { size: 12 })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "tb-btn close", title: "关闭", onClick: () => window.api.windowClose(), children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 15 }) })
    ] })
  ] });
}
...
const usePlaylistStore = create((set, get) => ({
  ...
  rename: (id, name) => {
    set({
      playlists: get().playlists.map(
        (p) => p.id === id && !p.builtin ? { ...p, name: name.trim().slice(0, 60) || p.name, updatedAt: Date.now() } : p
      )
    });
    persist(get);
  },
  ...
  addTracks: (id, trackIds) => {
    ...
    set({
      playlists: get().playlists.map((p) => {
        ...
        added = fresh.length;
        return fresh.length ? { ...p, trackIds: [...p.trackIds, ...fresh], updatedAt: Date.now() } : p;
      })
    });
    persist(get);
    return added;
  ...
  removeTracks: (id, trackIds) => {
    ...
    set({
      playlists: get().playlists.map(
        (p) => p.id === id ? { ...p, trackIds: p.trackIds.filter((t) => !rm.has(t)), updatedAt: Date.now() } : p
      )
    });
    persist(get);
  },
  toggleFavorite: (trackId) => {
    ...
    set({
      playlists: get().playlists.map(
        (p) => p.id === FAVORITES_ID ? { ...p, trackIds: next, updatedAt: Date.now() } : p
      )
    });
    persist(get);
    return !exists;
...
const useLibraryStore = create((set, get) => ({
  ...
  scan: async (roots) => {
    ...
    try {
      ...
      useUiStore.getState().toast("扫描失败: " + (err instanceof Error ? err.message : String(err)), "error");
      throw err;
    } finally {
      unsub();
...
function groupAlbums(tracks) {
  const map = /* @__PURE__ */ new Map();
  for (const t of tracks) {
    const key = `${t.artist.toLowerCase()}\0${t.album.toLowerCase()}`;
    const cur = map.get(key);
    if (cur) {
      cur.count++;
...
function buildSuggestions(tracks, q) {
  const query = q.trim();
  if (!query) return [];
  ...
  const artists = groupArtists(tracks).filter((a) => a.name.toLowerCase().includes(lower)).slice(0, 3);
  ...
  const albums = groupAlbums(tracks).filter((a) => a.album.toLowerCase().includes(lower)).slice(0, 3);
  ...
  const genres = [
    ...new Set(
      tracks.map((t) => t.genre).filter(Boolean).join("/").split("/").map((g) => g.trim()).filter(Boolean)
    )
  ].filter((g) => g.toLowerCase().includes(lower)).slice(0, 3);
...
const ICONS = {
  all: /* @__PURE__ */ jsxRuntimeExports.jsx(Search, { size: 14 }),
  song: /* @__PURE__ */ jsxRuntimeExports.jsx(Music2, { size: 14 }),
  artist: /* @__PURE__ */ jsxRuntimeExports.jsx(Users, { size: 14 }),
  album: /* @__PURE__ */ jsxRuntimeExports.jsx(Disc3, { size: 14 }),
  genre: /* @__PURE__ */ jsxRuntimeExports.jsx(Tag, { size: 14 })
};
function NavBar() {
  ...
  const [activeSel, setActiveSel] = reactExports.useState({
    query: globalSearch,
    index: 0
  });
  const activeIdx = activeSel.query === globalSearch ? activeSel.index : 0;
  const playlistView = view.type === "playlist";
  const suggestions = reactExports.useMemo(() => buildSuggestions(tracks, globalSearch), [tracks, globalSearch]);
  reactExports.useEffect(() => {
    if (!searchActive) return;
    const h = (e) => {
      if (!wrapRef.current?.contains(e.target)) setSearchActive(false);
    };
    document.addEventListener("pointerdown", h);
    return () => document.removeEventListener("pointerdown", h);
  }, [searchActive, setSearchActive]);
  const commit = (s) => executeSearch(s.criteria);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "nb", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "nb-tabs", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Library, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
            "音乐库"
          ]
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(ListMusic, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
            "歌单"
          ]
        }
      )
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "nb-search-wrap no-drag", ref: wrapRef, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `nb-search ${searchActive ? "focus" : ""}`, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Search, { size: 14, style: { color: "var(--text-faint)", flexShrink: 0 } }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
      ...
      searchActive && suggestions.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "search-drop", children: suggestions.map((s, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          onClick: () => commit(s),
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "search-item-icon", children: ICONS[s.kind] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "search-item-label", children: [
              s.label,
              s.sub && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "search-item-sub", children: [
                " ",
                s.sub
              ] })
            ] }),
            s.kind === "all" && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "search-item-enter", title: "回车", children: "↵" })
          ]
        },
        `${s.kind}-${s.label}-${i}`
      )) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "nb-right no-drag", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: "nb-tab",
          ...
          children: settings?.general.alwaysOnTop ? /* @__PURE__ */ jsxRuntimeExports.jsx(Pin, { size: 13, style: { color: "var(--accent)" } }) : /* @__PURE__ */ jsxRuntimeExports.jsx(PinOff, { size: 13 })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
...
function Sidebar() {
  ...
  const namingCancel = reactExports.useRef(false);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "sb", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sb-group-label", children: "音乐库" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        onClick: () => navTo({ type: "all" }),
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Library, { size: 15 }),
          "全部歌曲",
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "count", children: tracks.length || "" })
        ]
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Disc3, { size: 15 }),
          "专辑"
        ]
      }
    ),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sb-group-label", children: "智能列表" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        title: "根据你的常听歌手与曲风推荐",
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Sparkles, { size: 15, style: { color: "var(--accent)" } }),
          "为你推荐"
        ]
      }
    ),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(History, { size: 15 }),
          "最近播放"
        ]
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Flame, { size: 15, style: { color: "#ffb347" } }),
          "常听"
        ]
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sb-group-label", children: "我的歌单" }),
    fav && /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        className: `sb-item ${is({ type: "playlist", id: fav.id }) ? "active" : ""} ${dropTarget === fav.id ? "drop-target" : ""}`,
        onClick: () => navTo({ type: "playlist", id: fav.id }),
        ...dropHandlers(fav.id),
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Heart, { size: 15, style: { color: "#ff5f8a" } }),
          "收藏",
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "count", children: fav.trackIds.length || "" })
        ]
      }
    ),
    regular.map((p) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        className: `sb-item ${is({ type: "playlist", id: p.id }) ? "active" : ""} ${dropTarget === p.id ? "drop-target" : ""}`,
        onClick: () => navTo({ type: "playlist", id: p.id }),
        ...dropHandlers(p.id),
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(ListMusic, { size: 15 }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: p.name }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "count", children: p.trackIds.length || "" })
        ]
      },
      p.id
    )),
    naming ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      "input",
      {
    ...
    ) : /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "sb-new", onClick: () => setNaming(true), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Plus, { size: 13 }),
      "新建歌单"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sb-group-label", children: "快捷" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        className: "sb-item",
        ...
        children: [
          ...
          "拖拽导入 / 加歌",
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "count", children: "mp3 flac ape …" })
        ]
      }
    )
  ] });
...
class AudioEngine {
  ...
  bind(audio) {
    ...
    audio.addEventListener("waiting", () => this.emit("waiting"));
   ...
   */
  prepareContext(ctx, generation) {
    const attempt = async () => {
      if (typeof ctx.setSinkId === "function") {
        try {
          await ctx.setSinkId(DEFAULT_SINK_ID);
          if (this.ctxGeneration !== generation) {
            this.sinkSetup.boundSinkId = typeof ctx.sinkId === "string" ? ctx.sinkId : DEFAULT_SINK_ID;
  ...
  emit(ev, payload) {
    if (this.listeners.has(ev)) {
      for (const cb of Array.from(this.listeners.get(ev))) cb(payload);
    }
  }
...
function withQueueContext(requested, all, cap = 50) {
  if (requested.length !== 1 || all.length <= 1) return requested;
  const first = requested[0];
  const out = [first];
  const seen = /* @__PURE__ */ new Set([first.id]);
  const min = 8;
  const push = (list) => {
    for (const t of list) {
...
function shouldStopOnEnded(ctx) {
...
let loadSeq = 0;
let lastStatePush = 0;
function pushStateThrottled(force = false) {
  const now = Date.now();
...
function applySleepStop(from) {
  usePlayerStore.setState({ sleep: null, sleepRemaining: null });
  stopSleepTicker();
  audioEngine.pause();
  const label = from === "minute" ? "定时时间到" : from === "track" ? "当前歌曲已播完" : "当前队列已播完";
...
const usePlayerStore = create((set, get) => ({
  ...
  next: (manual = true) => {
    ...
    if (s.mode === "shuffle" && s.queue.length > 1) {
      let nextIndex2;
      do {
        nextIndex2 = Math.floor(Math.random() * s.queue.length);
      } while (nextIndex2 === s.index);
      void loadCurrentInternal(get, set, nextIndex2, true);
      return;
    }
    if (!manual && s.mode === "one") return;
    const nextIndex = sleepStop ? Math.min(s.index + 1, s.queue.length - 1) : (s.index + 1) % s.queue.length;
...
async function loadCurrentInternal(get, set, index, autoplay) {
  const seq = ++loadSeq;
  const { queue } = get();
...
function genresOf(track) {
  return track.genre.split("/").map((g) => g.trim()).filter((g) => g && g !== "未知");
}
function buildTasteProfile(tracks, stats) {
  const artistPlays = /* @__PURE__ */ new Map();
  const artistTracks = /* @__PURE__ */ new Map();
  ...
  const topArtists = [...artistPlays.entries()].map(([name, plays]) => ({ name, plays, trackCount: artistTracks.get(name) ?? 0 })).sort((a, b) => b.plays - a.plays);
  const topGenres = [...genrePlays.entries()].map(([name, plays]) => ({ name, plays })).sort((a, b) => b.plays - a.plays);
  const recentIds = Object.entries(stats).filter(([, s]) => s.count > 0).sort((a, b) => b[1].last - a[1].last).map(([id]) => id);
  return { topArtists, topGenres, playedIds, recentIds, totalPlays };
}
function recommendTracks(tracks, stats, limit = 30) {
...
function tasteSummary(profile) {
  if (profile.totalPlays === 0) return "用户尚未产生播放记录（可推荐新入库歌曲，或先了解其偏好）。";
  const artists = profile.topArtists.slice(0, 5).map((a) => `${a.name}(${a.plays}次)`).join("、");
  const genres = profile.topGenres.slice(0, 5).map((g) => `${g.name}(${g.plays}次)`).join("、");
...
function Cover({
  ...
  const [failedSrc, setFailedSrc] = reactExports.useState(null);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `cover-ph ${className ?? ""}`, "aria-label": alt, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Music2, {}) });
}
function TrackList({
  tracks,
  emptyTitle,
  ...
  const [anchorIndex, setAnchorIndex] = reactExports.useState(null);
  ...
  if (tracks.length === 0) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty-inner", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-icon", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { size: 26 }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-title", children: emptyTitle ?? "曲库还是空的" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-sub", children: emptySub ?? "扫描本地文件夹，或直接拖拽音频文件到窗口来导入" }),
      emptyAction
    ] }) });
  }
  const menuTrack = menu ? tracks.find((t) => t.id === menu.trackId) : null;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tl", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tl-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "#" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "标题" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "歌手" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "专辑" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { textAlign: "right" }, children: "时长" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", {})
    ] }),
    selected.size > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tl-selection-toolbar", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "tl-selected-count", children: [
        "已选 ",
        selected.size,
        " 首"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
            "播放选中"
          ]
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "btn", onClick: () => setPicker({ ids: selectedArr }), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(ListPlus, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
        "加入歌单"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Heart, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
            "收藏"
          ]
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", onClick: () => setSelected(/* @__PURE__ */ new Set()), children: "清空选择" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tl-rows", ref: parentRef, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { height: virtualizer.getTotalSize(), position: "relative" }, children: virtualizer.getVirtualItems().map((v) => {
      ...
      return /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          className: `tl-row ${isCurrent ? "current" : ""} ${t.missing ? "missing" : ""} ${isSelected ? "selected" : ""}`,
          draggable: !t.missing,
          onDragStart: (e) => {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tl-idx", children: isCurrent && isPlaying ? /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "eq", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", {}),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", {}),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", {})
            ] }) : v.index + 1 }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tl-title-cell", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tl-cover", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Cover, { src: t.coverPath, alt: t.title }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tl-name", children: t.title })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tl-artist", children: t.artist }),
            reasonFor ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tl-reason", children: reasonFor(t) ?? t.album }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tl-album", children: t.album }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tl-dur", children: formatTime(t.duration) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(FavoriteHeart, { trackId: t.id })
          ]
        },
        t.id
      );
    }) }) }),
    menu && menuTrack && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ctx-menu", style: { left: menu.x, top: menu.y }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          className: "ctx-item",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          className: "ctx-item",
          onClick: () => {
            const ids = selected.has(menuTrack.id) && selected.size > 1 ? selectedArr : [menuTrack.id];
            setPicker({ ids });
            setMenu(null);
          },
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(ListPlus, { size: 13 }),
            " ",
            selected.has(menuTrack.id) && selected.size > 1 ? `添加到歌单（${selected.size} 首）` : "添加到歌单"
          ]
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        ...
        "button",
        {
          ...
          onClick: () => {
            if (selected.has(menuTrack.id) && selected.size > 1) {
              ...
              useUiStore.getState().toast(`已从曲库移除 ${selected.size} 首（不删除源文件）`, "info");
            } else {
              void useLibraryStore.getState().remove([menuTrack.id]);
              useUiStore.getState().toast(`已从曲库移除「${menuTrack.title}」（不删除源文件）`, "info");
            }
            setSelected(/* @__PURE__ */ new Set());
            setMenu(null);
          },
    ...
    picker && /* @__PURE__ */ jsxRuntimeExports.jsx(PlaylistPicker, { ids: picker.ids, onClose: () => setPicker(null) })
  ] });
}
function FavoriteHeart({ trackId }) {
  const fav = usePlaylistStore(
    (s) => s.playlists.find((p) => p.builtin === "favorites")?.trackIds.includes(trackId)
  );
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsx(Heart, { size: 14, fill: fav ? "currentColor" : "none" })
    }
  );
}
function PlaylistPicker({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overlay", onClick: onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal", style: { width: 380 }, onClick: (e) => e.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-head", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-title", children: "添加到歌单" }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-body", style: { maxHeight: 320, overflowY: "auto" }, children: [
      playlists.map((p) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          onClick: () => addTo(p.id),
          children: [
            p.builtin ? /* @__PURE__ */ jsxRuntimeExports.jsx(Heart, { size: 14, style: { color: "#ff5f8a" } }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ListPlus, { size: 14 }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { flex: 1, textAlign: "left" }, children: p.name }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "count", children: p.trackIds.length })
          ]
        },
        p.id
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8, marginTop: 10 }, children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn primary", onClick: createAndAdd, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Plus, { size: 14 }) })
      ] })
    ] })
  ] }) });
}
...
const STORAGE_KEY = "nebula.lyricsoffset";
...
function setOffset(trackId, seconds) {
  if (!trackId) return 0;
...
function adjustOffset(trackId, delta = OFFSET_STEP) {
  if (!trackId) return 0;
  return setOffset(trackId, getOffset(trackId) + delta);
}
function clearOffset(trackId) {
  setOffset(trackId, 0);
}
...
function hasOffset(seconds) {
  return Math.abs(clamp(seconds)) > EPSILON;
}
function subscribeOffsets(onChange) {
...
function getOffsetSnapshot(trackId) {
...
function LyricsPanel() {
  ...
  const [load, setLoad] = reactExports.useState(null);
  const containerRef = reactExports.useRef(null);
  const activeRef = reactExports.useRef({ trackId: null, index: -1 });
  const offset = reactExports.useSyncExternalStore(subscribeOffsets, getOffsetSnapshot(currentId));
  reactExports.useEffect(() => {
    if (!current) return;
    let alive = true;
    const trackId = current.id;
    window.api.lyricsGet({ path: current.path, mtime: current.mtime, size: current.size }).then((r) => {
  ...
  reactExports.useEffect(() => {
    ...
    const el = container.querySelector(`[data-i="${active}"]`);
    if (!el) return;
    const top = el.offsetTop - container.clientHeight / 2 + el.offsetHeight / 2;
    container.scrollTo({ top, behavior: "smooth" });
  }, [active, currentId]);
  if (!current) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "lyrics-wrap", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "lyrics-empty", children: "播放歌曲后，这里会显示同步歌词" }) });
  }
  if (!loaded) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "lyrics-wrap", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "lyrics-empty", children: "正在读取歌词…" }) });
  }
  if (lines.length === 0 || source === "none") {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "lyrics-wrap", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "lyrics-empty", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(MicVocal, { size: 22, style: { marginBottom: 10, color: "var(--text-faint)" } }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: "未找到歌词" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { fontSize: 11, marginTop: 6, color: "var(--text-faint)" }, children: [
        "将同名 ",
        /* @__PURE__ */ jsxRuntimeExports.jsx("code", { children: ".lrc" }),
        " 文件放到音频旁，或使用内嵌歌词"
      ] })
    ] }) });
  }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: panelStyle, children: [
      ...
      "div",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
              className: tuned ? "chip-badge" : void 0,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "lyrics-wrap", ref: containerRef, children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { height: "45%" } })
    ] })
  ] });
}
function Spectrum() {
  ...
  const current = usePlayerStore((s) => s.current);
  const currentId = current?.id ?? null;
  const [waveState, setWaveState] = reactExports.useState(null);
  ...
  reactExports.useEffect(() => {
    ...
    const drawWaveform = (w, h, t, nowMs) => {
      const cur = usePlayerStore.getState().current;
      ...
      const dur2 = Math.max(1, cur.duration || env.length / 20);
      const n = env.length;
      const tClamped = Math.min(dur2, t);
      const W = Math.min(30, dur2);
      const winStart = Math.max(0, Math.min(tClamped - W, dur2 - W));
      const winEnd = Math.min(dur2, winStart + W);
      const startIdx = Math.max(0, Math.floor(winStart / dur2 * n));
      const endIdx = Math.min(n - 1, Math.ceil(winEnd / dur2 * n));
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("canvas", { ref: canvasRef, className: "spectrum" });
}
function NowPlaying() {
  const current = usePlayerStore((s) => s.current);
  const isPlaying = usePlayerStore((s) => s.isPlaying);
  ...
  if (!current) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("aside", { className: "detail", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty", style: { minHeight: 0, flex: 1 }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-title", children: "双击歌曲开始播放" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-sub", children: "播放后此处将展示封面、歌曲信息、频谱与歌词" })
    ] }) }) });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: `detail ${tab === "lyrics" ? "lyrics-mode" : ""}`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-tabs", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
    ...
    tab === "lyrics" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-name", style: { marginTop: 10 }, children: current.title }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-artist", children: current.artist }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(LyricsPanel, {})
    ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-cover", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Cover, { src: current.coverPath, alt: current.title }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-name", children: current.title }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-artist", children: current.artist }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-album", children: current.album }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-badge", children: [
        current.genre && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chip-badge", children: current.genre }),
        current.year ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chip-badge", children: current.year }) : null,
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chip-badge", children: formatTime(current.duration) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chip-badge", children: current.ext.toUpperCase() })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "spectrum-wrap", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "spectrum-title", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: direct ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "var(--accent-purple)" }, children: "WAVEFORM · 30s" }) : isPlaying ? "LIVE SPECTRUM" : "IDLE" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatTime(currentTime) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Spectrum, {})
      ] })
    ] })
  ] });
}
function MainView() {
  ...
  const title = view.type === "all" ? "全部歌曲" : view.type === "search" ? "搜索结果" : view.type === "artists" ? "歌手" : view.type === "albums" ? "专辑" : view.type === "folders" ? "本地文件夹" : view.type === "artist" ? view.artist : view.type === "album" ? view.key.split("\0")[1] ?? "专辑" : view.type === "folder" ? view.folderId.split(/[\\/]/).pop() ?? "文件夹" : view.type === "recent-added" ? "最近添加" : view.type === "recently-played" ? "最近播放" : view.type === "most-played" ? "常听" : view.type === "for-you" ? "为你推荐" : "歌单";
  const sub = view.type === "all" || view.type === "search" ? [
  ...
  ).join(" · ") : view.type === "playlist" ? `${shown.length} 首` : view.type === "artist" ? `${shown.length} 首歌曲` : view.type === "album" ? `${shown.length} 首歌曲` : view.type === "folder" ? `${shown.length} 首歌曲` : view.type === "recent-added" ? "按导入时间 · 最近 50 首" : view.type === "recently-played" ? "按播放时间 · 最近 30 首" : view.type === "most-played" ? "按播放次数 · 最多 30 首" : view.type === "for-you" ? "基于你的常听歌手与曲风 · 本地计算，离线可用" : "";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mv", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mv-list", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mv-header", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mv-title", children: title }),
          sub && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mv-sub", children: sub })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mv-actions", children: [
          view.type === "all" && /* @__PURE__ */ jsxRuntimeExports.jsx(ScanButton, {}),
          isTrackView(view) && /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "select",
            {
              className: "sort-select",
              value: sortKey,
              onChange: (e) => useUiStore.getState().setSortKey(e.target.value),
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "default", children: "默认排序" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "title", children: "按歌名" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "artist", children: "按歌手" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "album", children: "按专辑" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "duration", children: "按时长" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "addedAt", children: "按添加时间" })
              ]
            }
          ),
          shown.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              className: "btn",
          ...
          view.type === "playlist" && /* @__PURE__ */ jsxRuntimeExports.jsx(PlaylistActions, { view, shown }),
          scanning && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "mv-sub", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(RefreshCw, { size: 12, className: "muted-spin", style: { verticalAlign: "-2px" } }),
            "扫描中 ",
            scanProgress ? `${scanProgress.current}/${scanProgress.total}` : ""
          ] })
        ] })
      ] }),
      view.type === "artists" ? /* @__PURE__ */ jsxRuntimeExports.jsx(ArtistGrid, { tracks }) : view.type === "albums" ? /* @__PURE__ */ jsxRuntimeExports.jsx(AlbumGrid, { tracks }) : view.type === "folders" ? /* @__PURE__ */ jsxRuntimeExports.jsx(FolderGrid, { tracks }) : /* @__PURE__ */ jsxRuntimeExports.jsx(
        TrackList,
        {
          tracks: shown,
          reasonFor: reasons ? (t) => reasons[t.id] : void 0,
          emptyTitle: view.type === "search" ? "没有匹配的歌曲" : view.type === "playlist" ? "歌单还是空的" : tracks.length > 0 ? "没有匹配的歌曲" : "曲库还是空的",
          emptySub: view.type === "playlist" ? "右键歌曲 →「添加到歌单」，或把歌曲直接拖到左侧歌单" : tracks.length > 0 ? "换个关键词试试，或扫描更多文件夹" : "选择本地音乐文件夹，开始构建你的曲库",
          emptyAction: tracks.length === 0 && view.type !== "playlist" ? /* @__PURE__ */ jsxRuntimeExports.jsx(ScanButton, { big: true }) : void 0
        }
      )
    ] }),
...
function resolveTracks(view, tracks) {
  switch (view.type) {
    case "all":
      return tracks;
    case "search":
    ...
    case "album": {
      const [artist, album] = view.key.split("\0");
    ...
    case "playlist": {
      ...
      return p.trackIds.map((id) => map[id]).filter((t) => Boolean(t));
    }
    case "recent-added":
      return [...tracks].filter((t) => !t.missing).sort((a, b) => (b.addedAt ?? 0) - (a.addedAt ?? 0)).slice(0, 50);
    case "recently-played":
    case "most-played": {
      const stats = getPlayStats();
      const played = tracks.filter((t) => !t.missing && stats[t.id]).map((t) => ({ t, stat: stats[t.id] }));
      played.sort(
        (a, b) => view.type === "recently-played" ? b.stat.last - a.stat.last : b.stat.count - a.stat.count
      );
...
function ScanButton({ big }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "button",
    {
      ...
      disabled: scanning,
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(FolderPlus, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
        scanning ? `扫描中 ${scanProgress ? Math.round(scanProgress.current / Math.max(1, scanProgress.total) * 100) : 0}%` : big ? "扫描音乐文件夹" : "添加音乐"
      ]
    }
  );
}
function ArtistGrid({ tracks }) {
  const navTo = useUiStore((s) => s.navTo);
  const artists = groupArtists(tracks);
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "card-grid", children: artists.map((a) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      className: "card",
      onClick: () => navTo({ type: "artist", artist: a.name }),
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-cover artist-main", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Cover, { src: a.covers[0] ?? null, alt: a.name }),
          a.covers.length > 1 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "artist-badges", children: [
            a.covers.slice(1, 3).map((c, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "artist-mini", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Cover, { src: c, alt: `${a.name} ${i + 2}` }) }, `${c}-${i}`)),
            a.covers.length > 3 && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "artist-more", children: [
              "+",
              a.covers.length - 3
            ] })
          ] })
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-body", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "card-title", children: a.name }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-sub", children: [
            a.count,
            " 首歌曲"
          ] })
        ] })
...
function AlbumGrid({ tracks }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "card-grid", children: albums.map((a) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card", onClick: () => navTo({ type: "album", key: a.key }), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "card-cover", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Cover, { src: a.cover, alt: a.album }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-body", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "card-title", children: a.album }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-sub", children: [
        a.artist,
        " · ",
        a.count,
...
function FolderGrid({ tracks }) {
  const navTo = useUiStore((s) => s.navTo);
  const folders = groupFolders(tracks);
  if (folders.length === 0) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty-inner", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-icon", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Rss, { size: 26 }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-title", children: "还没有导入任何文件夹" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-sub", children: "点击右上角「添加音乐」扫描本地文件夹" })
    ] }) });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "card-grid", children: folders.map((f) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      className: "card",
      onClick: () => navTo({ type: "folder", folderId: f.folderId }),
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "card-cover", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Cover, { src: null, alt: f.folderName }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-body", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "card-title", children: f.folderName }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-sub", children: [
            f.count,
            " 首歌曲"
          ] })
...
function PlaylistActions({
  ...
  if (p.builtin) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
          "清空"
        ]
      }
    );
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    renaming && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overlay", onClick: () => setRenaming(false), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal", style: { width: 360 }, onClick: (e) => e.stopPropagation(), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-head", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-title", children: "重命名歌单" }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-body", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 14 }, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", onClick: () => setRenaming(false), children: "取消" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Pencil, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
          "重命名"
        ]
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
          "删除歌单"
        ]
      }
    ),
    shown.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "btn", onClick: () => void usePlayerStore.getState().playTracks(shown, 0), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
      "播放"
    ] })
  ] });
...
function PlayerBar() {
  ...
  const cycleMode = () => {
    ...
    const next = m === "list" ? "one" : m === "one" ? "shuffle" : "list";
    usePlayerStore.getState().setMode(next);
  };
  const ModeIcon = mode === "one" ? Repeat1 : mode === "shuffle" ? Shuffle : Repeat;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pb", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        className: "pb-cover",
        title: current ? `正在播放：${current.title} — ${current.artist}` : "未在播放",
        children: current ? /* @__PURE__ */ jsxRuntimeExports.jsx(Cover, { src: current.coverPath, alt: current.title }) : null
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pb-meta", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pb-title", children: current?.title ?? "未在播放" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pb-artist", children: current?.artist ?? "NEBULA PLAYER" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pb-center", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pb-buttons", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            className: "pb-btn",
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsx(SkipBack, { size: 17 })
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            disabled: !current,
            children: isLoading ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "spinner", style: { width: 16, height: 16, borderWidth: 2 } }) : isPlaying ? /* @__PURE__ */ jsxRuntimeExports.jsx(Pause, { size: 18, fill: "currentColor" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { size: 18, fill: "currentColor", style: { marginLeft: 2 } })
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsx(SkipForward, { size: 17 })
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            onClick: cycleMode,
            children: /* @__PURE__ */ jsxRuntimeExports.jsx(ModeIcon, { size: 15 })
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(ProgressBar, {})
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pb-right", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(SleepTimer, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: "pb-btn",
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(Captions, { size: 16 })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(VolumeControl, {})
    ] })
  ] });
}
function SleepTimer() {
  ...
  const ref = reactExports.useRef(null);
  const [menuLeft, setMenuLeft] = reactExports.useState(null);
  reactExports.useEffect(() => {
    if (!open) return;
    const id = setInterval(() => {
      const s = usePlayerStore.getState().sleep;
      setMenuLeft(
        s && s.deadline !== null ? Math.max(0, Math.ceil((s.deadline - Date.now()) / 1e3)) : null
      );
    }, 1e3);
  ...
  const title = sleep ? `睡眠定时器：${sleepModeLabel(sleep)}${sleep.mode === "minute" && countdown ? `（剩余 ${countdown}）` : ""}` : "睡眠定时器：未启用";
  const pick = (mode, mins) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "sleep-wrap", ref, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        onClick: () => setOpen((v) => !v),
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Moon, { size: 16, fill: sleep ? "currentColor" : "none" }),
          countdown && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "sleep-badge", children: countdown })
        ]
      }
    ),
    open && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "sleep-menu", role: "menu", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "sleep-menu-title", children: "睡眠定时器" }),
      SLEEP_MINUTES.map((m) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          role: "menuitem",
          ...
          children: [
            ...
            minutes === m && menuCountdown && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "sleep-item-hint", children: menuCountdown })
          ]
        },
        m
      )),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Music, { size: 14 }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "播完当前歌曲后暂停" })
          ]
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          onClick: () => pick("queue"),
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(ListEnd, { size: 14 }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "播完当前队列后暂停" })
          ]
        }
      ),
      sleep && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
...
function ProgressBar() {
  ...
  const isPlaying = usePlayerStore((s) => s.isPlaying);
  const isLoading = usePlayerStore((s) => s.isLoading);
  const [dragRatio, setDragRatio] = reactExports.useState(null);
  const ref = reactExports.useRef(null);
  const [live, setLive] = reactExports.useState(0);
  reactExports.useEffect(() => {
    if (!isPlaying) return;
    let raf = 0;
    const tick = () => {
  ...
  const shownTime = dragRatio !== null ? dragRatio * duration : isPlaying ? live : usePlayerStore.getState().currentTime;
  const time = Number.isFinite(shownTime) ? shownTime : 0;
  const ratio = duration > 0 ? Math.min(1, time / duration) : 0;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "progress-wrap", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "progress-time", children: formatTime(time) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ref,
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pbar-track", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pbar-fill", style: { width: `${ratio * 100}%` } }) })
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "progress-time", children: formatTime(duration) })
  ] });
}
function VolumeControl() {
  ...
  const Icon2 = muted || volume === 0 ? VolumeX : volume < 0.5 ? Volume1 : Volume2;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "vol-track", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            className: "vol-fill",
...
function toolsForLLM() {
  return MUSIC_TOOLS.map((t) => ({ type: "function", function: t }));
}
const DESTRUCTIVE_TOOLS = /* @__PURE__ */ new Set(["remove_from_playlist"]);
...
const CONFIRM_TOKEN_SEP = "\0";
function tokenPart(value) {
  return value.split(CONFIRM_TOKEN_SEP).join("_").split("|").join("_");
}
function mintConfirmToken(deps, name) {
  return [CONFIRM_TOKEN_PREFIX, deps.runId, name, deps.toolCallId, deps.nonce].map(tokenPart).join(CONFIRM_TOKEN_SEP);
}
function confirmTokenMatches(token, deps, name) {
  if (!token) return false;
  const parts = token.split(CONFIRM_TOKEN_SEP);
  return parts.length === 5 && parts[0] === CONFIRM_TOKEN_PREFIX && parts[1] === tokenPart(deps.runId) && parts[2] === tokenPart(name) && parts[3] === tokenPart(deps.toolCallId) && parts[4] === tokenPart(deps.nonce);
}
function strArg(args, key) {
  const v = args[key];
  return typeof v === "string" ? v.trim() : "";
}
function idListArg(args, key) {
  return (Array.isArray(args[key]) ? args[key] : []).map(String).filter(Boolean);
}
function describeTracks(ids) {
  const map = useLibraryStore.getState().map;
  const names = ids.map((id) => map[id]?.title ? `《${map[id].title}》` : id);
  const shown = names.slice(0, 3).join("、");
  return names.length > 3 ? `${shown} 等 ${names.length} 首` : shown;
}
function summarizeDestructiveTool(name, args) {
...
function destructiveGuard(name, args, deps) {
  ...
  return {
    ok: false,
    needsConfirm: true,
    content: `等待用户确认：${summary}。该操作尚未执行，用户确认后会用同一次调用继续执行，请不要重复调用同一个工具。`,
    chip: `待确认：${summary}`,
    confirmSummary: summary,
    confirmToken: deps ? mintConfirmToken(deps, name) : void 0
  };
}
function resolvePlaylist(nameOrId) {
  ...
  const fuzzy = list.find(
    (p) => !p.builtin && (p.name.toLowerCase().includes(lower) || lower.includes(p.name.toLowerCase()))
  );
...
async function executeTool(name, rawArgs, deps) {
  ...
  const fail = (msg) => ({ ok: false, content: msg, chip: msg });
  ...
  switch (name) {
    ...
    case "play_tracks": {
      ...
      const requested = ids.map((id) => library.map[id]).filter((t) => Boolean(t));
    ...
    case "set_volume": {
      const pct = args.percent !== void 0 ? Number(args.percent) : args.volume !== void 0 ? Number(args.volume) * 100 : NaN;
    ...
    case "seek": {
      const dur2 = player.duration || player.current?.duration || 0;
      let target;
      if (args.seconds !== void 0) target = Number(args.seconds);
      else if (args.percent !== void 0) target = Number(args.percent) / 100 * dur2;
    ...
    case "get_library_summary": {
      ...
      const albums = new Set(tracks.map((t) => t.artist + "\0" + t.album)).size;
      const genres = {};
      for (const t of tracks) {
      ...
      const topGenres = Object.entries(genres).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([k, v]) => ({ genre: k, count: v }));
      return {
        ok: true,
    ...
    case "create_playlist": {
      ...
      if (!name2) return fail("歌单名称为空");
      const p = playlist.create(name2);
    ...
    case "get_listening_profile": {
      ...
      return {
        ...
        content: JSON.stringify({
          ...
          topArtists: profile.topArtists.slice(0, 10).map((a) => ({ name: a.name, plays: a.plays })),
          topGenres: profile.topGenres.slice(0, 10),
          recentlyPlayed: profile.recentIds.slice(0, 10).map((id) => library.map[id]?.title).filter(Boolean)
        }),
        chip: profile.totalPlays > 0 ? `口味画像：${profile.topArtists[0]?.name ?? "—"} 等` : "暂无播放记录"
      };
    }
...
function buildSystemPrompt() {
  ...
  const playlist = usePlaylistStore.getState();
  const cur = player.current;
  const modeLabel = player.mode === "list" ? "列表循环" : player.mode === "one" ? "单曲循环" : "随机播放";
  const playing = cur ? `正在播放《${cur.title}》- ${cur.artist}（第 ${Math.floor(player.currentTime)}s / ${Math.floor(player.duration || cur.duration)}s）` : "当前未播放歌曲";
...
function persistHistory(messages) {
  ...
  saveTimer = setTimeout(() => {
    void window.api.chatSave(messages.slice(-40 - 10));
  }, 300);
}
let convId = "";
...
const useChatStore = create((set, get) => ({
  ...
  send: async () => {
    ...
    const userLlm = { role: "user", content: text };
    const baseHistory = history.slice(-40).map((m) => ({ role: m.role, content: m.content }));
    const conv = uid();
    convId = conv;
    const resume = {
      convId: conv,
...
function settleRun(set, get, accumulated, chips) {
  if (get().pendingConfirm) return;
  finishRun(set, get);
  finalize(set, get, accumulated.trim(), chips);
}
...
function replaceToolMessage(chain, toolCallId, message) {
...
async function completeToolCalls(resume, roundText, incomingToolCalls, set, get) {
  ...
  let parked = false;
  const answerToolCall = (toolCallId, content) => {
...
async function runFrom(resume, set, get) {
...
function handleRunError(set, get, err, accumulated, chips) {
...
const ASSISTANT_TITLE$1 = "薇拉 Vela";
const SUGGESTIONS$1 = [
  "放点适合雨天的歌",
  "把这首歌加进夜跑歌单",
  "这张专辑是什么风格？",
...
function ChatPanel() {
  ...
  const streamingBubble = busy ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "anim-in", children: [
    chips.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chat-chips", children: chips.map((c, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chat-chip", children: c }, i)) }),
    streamRaw && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-msg ai", children: [
      streamRaw.slice(0, streamShown),
      streamShown < streamRaw.length && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "caret" })
    ] })
  ] }, "streaming") : null;
  const confirmBar = pendingConfirm ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "anim-in", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 6, color: "var(--text)" }, children: pendingConfirm.summary }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chat-chips", style: { marginTop: 6 }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chat-chip err", children: "该操作尚未执行" }) })
  ] }, "confirm") : null;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `chat ${collapsed ? "collapsed" : ""} ${busy ? "chat-busy" : ""}`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-head", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-title", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chat-status" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Bot, { size: 15, style: { color: "var(--accent)" } }),
        ASSISTANT_TITLE$1
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-head-actions", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            children: collapsed ? /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronUp, { size: 15 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronDown, { size: 15 })
          }
        )
      ] })
    ] }),
    !collapsed && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chat-body", ref: bodyRef, children: bubbles.length === 0 && !busy ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chat-empty", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Sparkles, { size: 20, style: { marginBottom: 8, color: "var(--accent-purple)" } }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        "我是",
        ASSISTANT_TITLE$1,
        "，你的音乐领航员 —— 用一句话指挥播放器，或随便聊聊音乐。"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: 11, marginTop: 10 }, children: "试试：" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", flexDirection: "column", gap: 6, marginTop: 8 }, children: SUGGESTIONS$1.map((s) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
      ...
      bubbles.map((m) => /* @__PURE__ */ jsxRuntimeExports.jsx(Bubble, { bubble: m }, m.id)),
      streamingBubble,
      confirmBar
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-input", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        ...
        {
          ref: taRef,
          placeholder: pendingConfirm ? "请先确认或取消上面的操作…" : collapsed ? "输入指令或问题…（点击展开查看回复）" : "输入指令或问题，Enter 发送…",
          value: draft,
          rows: collapsed ? 1 : 2,
          disabled: false,
      ...
      busy ? /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(Square, { size: 14, fill: "currentColor" })
        }
      ) : /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
    ...
    !collapsed && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-hint", children: [
...
function Bubble({ bubble }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "anim-in", children: [
    bubble.chips && bubble.chips.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chat-chips", children: bubble.chips.map((c, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chat-chip", children: c }, i)) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `chat-msg ${bubble.role === "user" ? "user" : "ai"}`, children: bubble.content || " " })
  ] });
}
function SettingsModal() {
  const [tab, setTab] = reactExports.useState("general");
  const close = () => useUiStore.getState().setSettingsOpen(false);
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overlay", onClick: close, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal", onClick: (e) => e.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-head", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-title", children: "设 置" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "tb-btn", onClick: close, children: "✕" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ...
        children: [
          ...
          ["ai", "AI 配置"],
          ["about", "关于"]
        ].map(([t, label]) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-body", children: [
      ...
      tab === "ai" && /* @__PURE__ */ jsxRuntimeExports.jsx(AiTab, {}),
      tab === "about" && /* @__PURE__ */ jsxRuntimeExports.jsx(AboutTab, {})
    ] })
  ] }) });
}
function CheckRow({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "form-check", children: [
...
function GeneralTab() {
  ...
  if (!settings) return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-sub", children: "加载中…" });
  const general = settings.general;
  const addFolders = async () => {
    const folders = await window.api.selectFolders();
    if (folders.length === 0) return;
    const merged = Array.from(/* @__PURE__ */ new Set([...general.scanFolders, ...folders]));
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "form-label", style: { marginBottom: 4 }, children: "行为" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      CheckRow,
      {
        label: "关闭窗口时最小化到系统托盘",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "form-label", style: { margin: "16px 0 8px" }, children: "扫描目录" }),
    general.scanFolders.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty-sub", style: { marginBottom: 8 }, children: [
      "尚未添加目录 ",
      tracks.length > 0 ? "（曲库由拖拽或「添加音乐」导入）" : ""
    ] }),
    general.scanFolders.map((f) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "span",
        {
          style: {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          title: "移除（不删除歌曲）",
          onClick: () => void useSettingsStore.getState().updateGeneral({
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "btn", onClick: () => void addFolders(), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(FolderPlus, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
        "添加目录并扫描"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "btn", onClick: () => void rescanAll(), disabled: busy || scanning, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(RefreshCw, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
        "重新扫描全部"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "btn", onClick: () => void cleanupMissing(), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { size: 13, style: { verticalAlign: "-2px", marginRight: 5 } }),
        "清理缺失文件"
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "form-label", style: { margin: "18px 0 6px" }, children: "曲库概况" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty-sub", children: [
      "共 ",
      stats.total,
      " 首 · ",
      stats.artists,
...
function AiTab() {
  ...
  const toast = useUiStore((s) => s.toast);
  const [seededFrom, setSeededFrom] = reactExports.useState(null);
  ...
  if (!settings) return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-sub", children: "加载中…" });
  const save = async () => {
    await useSettingsStore.getState().setApi({ baseURL, model, apiKey: key || void 0 });
    setKey("");
    setDirty(false);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-sub", style: { marginBottom: 14 }, children: "兼容 OpenAI Chat Completions 接口（OpenAI / DeepSeek / Moonshot / 通义 / GLM 等）。 仅 AI 功能需要联网，本地播放完全离线可用。" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "form-label", children: "接口地址（Base URL）" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          className: "form-input",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "form-label", children: "模型名称" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          className: "form-input",
      ...
      suggestions.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "model-suggest", children: [
        suggestions.includes(model) ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-sub", style: { marginTop: 6 }, children: "当前模型已在支持列表中 ✓" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-sub", style: { marginTop: 6 }, children: "接口支持以下模型，点选即可：" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", flexWrap: "wrap", gap: 6, marginTop: 6 }, children: suggestions.map((m) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "form-label", children: [
        "API Key ",
        settings.api.hasKey ? "（已保存，留空则不修改）" : ""
      ] }),
    ...
    settings.api.keyMode === "plain" && /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        className: "toast error",
        style: { marginBottom: 12, animation: "none", pointerEvents: "auto" },
        children: "当前系统安全存储不可用，API Key 以加密前处理方式保存在本地，请注意保管设备安全。"
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn primary", onClick: () => void save(), disabled: !dirty, children: "保存配置" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          className: "btn",
          ...
          children: [
            testing ? /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { size: 13, className: "muted-spin" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(PlugZap, { size: 13, style: { verticalAlign: "-2px" } }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { marginLeft: 5 }, children: "测试连接" })
          ]
        }
      ),
      settings.api.hasKey && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: "btn danger",
          onClick: () => {
...
function AboutTab() {
  ...
  const settings = useSettingsStore((s) => s.data);
  const [updateURL, setUpdateURL] = reactExports.useState("");
  const [updStatus, setUpdStatus] = reactExports.useState({ state: "idle" });
  const [busy, setBusy] = reactExports.useState(false);
  const toast = useUiStore((s) => s.toast);
  reactExports.useEffect(() => {
  ...
  reactExports.useEffect(() => window.api.onUpdateStatus(setUpdStatus), []);
  const saveURL = async () => {
  ...
  const statusText = updStatus.state === "idle" ? "" : updStatus.state === "not-configured" ? "请先保存上面的更新源地址" : updStatus.state === "checking" ? "正在检查更新…" : updStatus.state === "none" ? "已是最新版本 ✓" : updStatus.state === "available" ? `发现新版本 v${updStatus.version}` : updStatus.state === "downloading" ? `正在下载更新 ${updStatus.percent}%` : updStatus.state === "downloaded" ? `v${updStatus.version} 已下载完成，可重启安装` : updStatus.state === "error" ? `更新失败：${updStatus.message}` : "";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty-sub", style: { lineHeight: 2.2 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "gradient-text", style: { fontSize: 16, fontWeight: 700 }, children: "NEBULA Player" }),
        " ",
        "v",
        info?.version ?? "…"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: "深空黑科技风 · AI 本地音乐播放器" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        "平台：",
        info?.platform ?? "…"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10 }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          "日志目录：",
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "var(--text-dim)" }, children: "userData/logs" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", onClick: () => void window.api.openLogDir(), children: "打开日志目录" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        "数据目录：",
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { wordBreak: "break-all", color: "var(--text-dim)" }, children: info?.userDataPath ?? "…" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 8, color: "var(--text-faint)" }, children: "本地播放完全离线 · 仅 AI 对话联网 · 不读取不修改源文件 · API Key 系统级加密存储" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "form-label", style: { margin: "16px 0 6px" }, children: "自动更新 · 更新源地址（Generic 静态托管，需含 latest.yml 与安装包）" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", onClick: () => void saveURL(), children: "保存" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty-sub", style: { marginTop: 6 }, children: [
      "启动后 8 秒自动检查一次；也可手动检查。更新源内放置 ",
      /* @__PURE__ */ jsxRuntimeExports.jsx("code", { children: "latest.yml" }),
      " ",
      "和最新安装包即可。"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8, alignItems: "center", marginTop: 12 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          className: "btn primary",
      ...
      (updStatus.state === "available" || updStatus.state === "downloaded") && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: "btn",
          onClick: () => {
            if (updStatus.state === "available") {
              setBusy(true);
              void window.api.updateDownload().then(setUpdStatus).finally(() => setBusy(false));
            } else {
              window.api.updateInstall();
            }
          },
          disabled: busy,
          children: updStatus.state === "available" ? "下载更新" : "重启并安装"
        }
      ),
...
function Toasts() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "toasts", children: toasts.map((t) => /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
...
function MiniChat() {
  const [snap, setSnap] = reactExports.useState(EMPTY);
  ...
  const cmd = (c) => window.api.chatProxyCmd(c);
  const send = () => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mini-chat no-drag", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Bot, { size: 12, style: { color: "var(--accent)" } }),
          ASSISTANT_TITLE
        ]
      }
    ),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mini-chat-body", ref: bodyRef, children: [
      snap.bubbles.length === 0 && !showStream && !pending ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mini-chat-empty", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-chat-suggestions", children: SUGGESTIONS.map((s) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            className: "chat-chip",
        ...
        snap.bubbles.map((b) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          b.chips && b.chips.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chat-chips", children: b.chips.map((c, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chat-chip", children: c }, i)) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `chat-msg ${b.role === "user" ? "user" : "ai"}`, children: b.content || " " })
        ] }, b.id)),
        showStream && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          snap.chips.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chat-chips", children: snap.chips.map((c, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "chat-chip", children: c }, i)) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-msg ai", children: [
            streaming,
            snap.streamShown < snap.streamRaw.length && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "caret" })
          ] })
        ] })
      ] }),
      pending && /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          ...
          children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 5 }, children: pending.summary }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 6, marginTop: 8 }, children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", disabled: busy, onClick: () => cmd({ kind: "cancel" }), children: "取消" })
            ] })
          ]
        }
      )
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mini-chat-input", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "textarea",
        {
          ref: taRef,
          rows: 1,
          placeholder: pending ? "请先确认或取消上面的操作…" : busy ? "AI 正在回复…" : "Enter 发送 · Shift+Enter 换行",
          value: draft,
          onChange: (e) => setDraft(e.target.value),
          onKeyDown
        }
      ...
      busy ? /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(Square, { size: 13, fill: "currentColor" })
        }
      ) : /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(Bot, { size: 13 })
        }
      )
    ] })
  ] });
...
function MiniSearch({ onQueryChange } = {}) {
  ...
  const q = query.trim();
  const results = q ? searchTracks(cache2, { text: q }) : [];
  const rows = results.slice(0, MAX_ROWS);
  const playRow = (index) => {
    window.api.miniPlay({ ids: results.map((t) => t.id), index });
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mini-search no-drag", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mini-search-bar", children: [
    ...
    q ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-search-results", children: rows.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-search-row empty", children: !loaded ? "正在读取曲库…" : cache2.length === 0 ? "曲库为空，先在主窗口添加音乐" : "没有匹配的歌曲" }) : rows.map((t, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        onClick: () => playRow(i),
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mini-search-row-title", children: t.title }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mini-search-row-artist", children: t.artist }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mini-search-row-dur", children: dur(t.duration) })
        ]
      },
      t.id
    )) }) : loaded && cache2.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-search-hint", children: "曲库为空，先在主窗口添加音乐" })
  ] });
}
function clearLyricsHolders(...holders) {
  for (const holder of holders) holder.current = null;
}
function claimLyricsRequest(claimed, id) {
  if (claimed.current === id) return false;
  claimed.current = id;
  return true;
}
function acceptLyricsResponse(wanted, id) {
  return wanted.current === id;
}
function trackLyricsPush(wanted, id) {
  wanted.current = id;
}
function MiniPlayer() {
  ...
  const timeRef = reactExports.useRef({ t: 0, ts: 0 });
  const claimed = reactExports.useRef(null);
  const wanted = reactExports.useRef(null);
  const trackId = track?.id ?? null;
  const offset = reactExports.useSyncExternalStore(subscribeOffsets, getOffsetSnapshot(trackId));
  reactExports.useEffect(() => {
    const off = window.api.onMiniState((p) => {
      ...
      void window.api.lyricsGet({ path: incoming.path, mtime: incoming.mtime, size: incoming.size }).then((r) => {
  ...
  reactExports.useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (!s) return;
      const now = Date.now();
      const dt = (now - timeRef.current.ts) / 1e3;
  ...
  const toggleExpanded = () => {
    const next = !expanded;
    setExpanded(next);
    void window.api.miniSetExpanded(next).then((size) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mini-head drag", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-cover", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Cover, { src: track?.coverPath ?? null, alt: track?.title ?? "" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mini-meta", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-title", children: track?.title ?? "未在播放" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-artist", children: track?.artist ?? "NEBULA PLAYER" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: expanded ? /* @__PURE__ */ jsxRuntimeExports.jsx(Minimize2, { size: 13 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Maximize2, { size: 13 })
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "mini-close no-drag", title: "隐藏", onClick: () => window.api.miniClose(), children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 14 }) })
        ] }),
        expanded && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-search-zone", children: /* @__PURE__ */ jsxRuntimeExports.jsx(MiniSearch, { onQueryChange: setSearching }) }),
        !searching && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-lyric", children: lines.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-lyric-line none", children: track ? "暂无歌词 — 把同名 .lrc 放到音频旁" : "播放歌曲后显示同步歌词" }) : active >= 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
          expanded && active > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-lyric-line prev", children: lines[active - 1].text }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-lyric-line active", children: lines[active].text }),
          active < lines.length - 1 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-lyric-line next", children: lines[active + 1].text })
        ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mini-lyric-line prev", children: lines[0]?.text ?? "" }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mini-controls no-drag", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "mini-btn", title: "上一曲", onClick: () => window.api.miniCommand("prev"), children: /* @__PURE__ */ jsxRuntimeExports.jsx(SkipBack, { size: 15 }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: state?.isPlaying ? /* @__PURE__ */ jsxRuntimeExports.jsx(Pause, { size: 17, fill: "currentColor" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { size: 17, fill: "currentColor" })
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "mini-btn", title: "下一曲", onClick: () => window.api.miniCommand("next"), children: /* @__PURE__ */ jsxRuntimeExports.jsx(SkipForward, { size: 15 }) })
        ] }),
        expanded && /* @__PURE__ */ jsxRuntimeExports.jsx(MiniChat, {})
      ]
    }
  );
}
const DEFAULT_BUBBLE_LIMIT = 20;
...
function buildChatSnapshot(state, opts = {}) {
...
function installMainWindowBridge() {
  const offPlay = window.api.onMiniPlay(({ ids, index }) => {
    const lib = useLibraryStore.getState();
    const list = (Array.isArray(ids) ? ids : []).map((id) => lib.map[id]).filter((t) => !!t && !t.missing);
    if (list.length === 0) {
      useUiStore.getState().toast("这些歌曲已不在曲库中", "info");
      return;
    }
    const at = Number.isFinite(index) ? Math.min(Math.max(0, Math.floor(index)), list.length - 1) : 0;
...
function App() {
  if (IS_MINI) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx(MiniPlayer, {});
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx(FullApp, {});
}
function FullApp() {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "div",
        {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { textAlign: "center" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: 18, letterSpacing: 6 }, className: "gradient-text", children: "NEBULA" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-root", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      ...
      {
        className: "shell",
        style: { "--bottom-h": chatCollapsed ? "128px" : "min(342px, 33vh)" },
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(TitleBar, {}),
          /* @__PURE__ */ jsxRuntimeExports.jsx(NavBar, {}),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "middle", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Sidebar, {}),
            /* @__PURE__ */ jsxRuntimeExports.jsx(MainView, {})
          ] }),
