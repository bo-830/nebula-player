...
function summarizeDestructiveTool(name, args) {
  ...
  if (name === "remove_from_playlist") {
    ...
    if (ids.length === 0) return `从${who}移除歌曲（未指定歌曲）`;
    return `从${who}移除 ${ids.length} 首歌曲：${describeTracks(ids)}`;
  }
  return `执行「${label}」`;
}
...
function buildSystemPrompt() {
  ...
  return [
    ...
    "5. 破坏性操作（把歌曲移出歌单等）需要用户确认：收到「等待用户确认」的工具结果时，说明将要执行的操作并请用户在聊天框确认或取消，不要重复调用同一个工具；用户确认后系统会继续，用户取消时不要重试。",
    "6. 一律使用简体中文回复；工具执行类回复尽量简洁，对话类回复可充分展开。"
  ].join("\n");
}
const MAX_ROUNDS = 6;
let saveTimer = null;
