...
interface StreamHandle {
   ...
   * lets those already-queued events run first, so the buffered deltas are never
   * dropped; `next()` reports the buffer, never the store snapshot, so a late
   * event can no longer be out-run by the sampling.
   */
  next: () => Promise<string>
}
...
function openStream(rid: string, seed: string): StreamHandle {
  ...
  const unsub = window.api.onChatChunk((p) => {
    // H1 CONTROL #2: accept chunks regardless of the stream-teardown flag
