...
/**
 * H1 — the typewriter reveal cursor must not invalidate the round's stream.
 *
 * Mechanism (reported by the reviewer, then reproduced here): the 22ms reveal
 * timer that advances `streamShown` 5 characters per tick used to call
 * `closeStream()` as soon as it caught up with `streamRaw`. `closeStream()`
 * resets the module-level `streamConvId`, and the chunk guard
 * (`closed || p.id !== rid || streamConvId !== rid`) uses that id as the
 * ownership check ⇒ after ONE catch-up every later delta of the SAME round was
 * dropped, so a slowly drip-fed / long answer was persisted truncated.
 *
 * `noclose` (drop the `closeStream()` in that branch) and `noguard` (never check
 * `streamConvId`) were the reviewer's controls: both keep the whole answer, which
 * proves the defect sits in exactly this coupling.
 *
 * TIMING MATTERS — a stray late delta arriving AFTER the round resolved is a
 * different scenario: there the stream is already closed on purpose and the
 * delta must be ignored (it may not leak into the next round's buffer). The bug
 * is about deltas that arrive while the round is STILL streaming but the reveal
 * cursor has already caught up. That is what these fixtures model: the round's
 * promise resolves only after the last delta, so real wall-clock timers are
 * required — with fake timers the 22ms cursor would never tick between two
 * deliveries and the catch-up branch could not be reached at all.
 */
describe('typewriter catch-up keeps the round alive (t58 / H1)', () => {
  const FIRST = 'AAAA'
  const SECOND = 'BBBB'
  const TOTAL = `${FIRST}${SECOND}` // 8 chars ⇒ the cursor catches up after ~2 ticks
  const SAMPLES = [100, 2000] as const
  const SAMPLE_MS = 5

  type Sample = { t: number; r: number; s: number; text: string }

  /**
   * Two deltas of the SAME round, `gap` ms apart, emitted from inside the
   * (still pending) `chat:complete` call — i.e. the round is in flight across
   * the whole drip, exactly like a real slow model.
   */
  function installDripWindow(gap: number): void {
    // ONE round, two sequential deltas: the inner array is the first call's steps
    installControlledWindow([
      [
        { text: FIRST, gapMs: 0 },
        { text: SECOND, gapMs: gap }
      ]
    ])
  }

  type DripStep = { text: string; gapMs: number }

  /**
   * Fake gateway whose `chat:complete` emits `steps` (one delta per step, each
   * `gapMs` after the previous one) and only then resolves, so the round stays
   * in flight across the whole drip. Every call consumes the next step list; a
   * call with no steps left answers with `''`.
   */
  function installControlledWindow(stepsPerCall: DripStep[][]): {
    straggler: (text: string) => void
  } {
    const glob = globalThis as unknown as { window: Record<string, unknown> }
    const handlers: Array<(p: { id: string; delta: string }) => void> = []
    /** ids of finished rounds, so a test can fire an OUT-OF-BAND straggler */
    ...
    glob.window = {
      api: {
        chatLoad: async () => ({ messages: [] as ChatMessage[] }),
        chatSave: async () => true,
        // `playlistStore.setState` schedules a debounced save (400ms). These H1
        // cases run on real timers long enough for it to fire, so the fake API
        // must answer it — otherwise the run reports an unhandled error.
        playlistSave: async () => true,
        chatAbort: (): void => {},
        onChatChunk: (cb: (p: { id: string; delta: string }) => void) => {
          handlers.push(cb)
          return (): void => {
        ...
        chatComplete: async (payload: { id: string }) => {
          const steps = stepsPerCall[call++] ?? []
          const deliver = (d: string): void => {
            for (const h of handlers.slice()) h({ id: payload.id, delta: d })
          }
          for (const step of steps) {
            if (step.gapMs > 0) {
              await new Promise<void>((resolve) => setTimeout(resolve, step.gapMs))
            }
            deliver(step.text)
          }
          doneIds.push(payload.id)
          return { id: payload.id, finishReason: 'stop', toolCalls: [] as LLMToolCall[] }
        }
      }
    }
    // NOTE: the straggler is emitted with the LAST finished round's id, i.e. a
    // delta whose `p.id` no longer matches the live round — see
    // `installControlledWindow`'s docblock above.
    return {
      /** deliver a delta on behalf of an already-finished round */
      straggler: (text: string): void => {
        const id = doneIds.at(-1) ?? ''
        for (const h of handlers.slice()) h({ id, delta: text })
      }
    }
  }

  /** drive one turn and record `streamRaw`/`streamShown`/bubble over real time */
  async function dripTurn(gap: number): Promise<{ samples: Sample[]; finalText: string }> {
    installDripWindow(gap)
    ...
    const watcher = setInterval(() => {
      ...
      samples.push({
        t: Date.now() - t0,
        r: s.streamRaw.length,
        s: s.streamShown,
        text: s.messages.at(-1)?.content ?? ''
      })
    }, SAMPLE_MS)

    await send('给我讲个很长的故事')
    // one more sample after the turn so the final state is recorded
    await new Promise<void>((resolve) => setTimeout(resolve, SAMPLE_MS))
    watching = false
    clearInterval(watcher)

    return { samples, finalText: useChatStore.getState().messages.at(-1)?.content ?? '' }
  }

  function assertNoTruncation(gap: number, r: { samples: Sample[]; finalText: string }): void {
    const timeline = r.samples
      .filter((s, i) => i === 0 || s.r !== r.samples[i - 1].r || s.s !== r.samples[i - 1].s)
      .map((s) => `${s.t}:r${s.r}/s${s.s}`)
      .join(' → ')
    const detail = `gap=${gap}ms timeline: ${timeline}`

    // the cursor was BEHIND the buffer while the round was still streaming
    const firstArrived = r.samples.find((s) => s.r >= FIRST.length)
    expect(firstArrived, `first delta observed | ${detail}`).toBeTruthy()
    expect(firstArrived!.s).toBeLessThan(firstArrived!.r)

    // …and it caught up with the first delta while the round was still in flight
    // (the 100ms arm cannot sample the post-second-delta state reliably, because
    // the round ends ~2ms later — so the catch-up is asserted from the timeline
    // and the acceptance of the SECOND delta is asserted from the final text,
    // which is the thing that actually reaches the user)
    const caughtUp = r.samples.find((s) => s.r === FIRST.length && s.s === FIRST.length)
    expect(caughtUp, `cursor caught up mid-round | ${detail}`).toBeTruthy()
    expect(caughtUp!.t).toBeLessThan(gap)

    // THE H1 ASSERTION, stated as the symptom: if the second delta was dropped by
    // a catch-up-invalidated stream, the persisted answer is exactly the first
    // half. It is not — the whole answer survived.
    expect(r.finalText, detail).toBe(TOTAL)
    expect(r.finalText, detail).not.toBe(FIRST)
    expect(useChatStore.getState().busy).toBe(false)

    // when the buffer is observable after the second delta (the slow arm gives it
    // a wide window), the cursor is mid-reveal again rather than stuck at the
    // catch-up position: the reveal really continues
    const secondBuffered = r.samples.find((s) => s.r >= TOTAL.length)
    if (secondBuffered) expect(secondBuffered.s).toBeLessThan(secondBuffered.r)
  }

  for (const gap of SAMPLES) {
    it(`keeps the whole answer across the catch-up (drip gap ${gap}ms)`, async () => {
      const r = await dripTurn(gap)
      assertNoTruncation(gap, r)
    })
  }

  /**
   * The other half of the same decision: keeping ownership for the whole round
   * must NOT keep it past the round. Once the turn ends (`finishRun` + `unsub`)
   * the stream is really closed, so a straggler delta of the old round is
   * dropped instead of being buffered into the NEXT round.
   */
  it('drops a straggler after the round ended (no leak into the next round)', async () => {
    const gw = installControlledWindow([[{ text: FIRST, gapMs: 0 }], [{ text: 'C', gapMs: 0 }]])

    await send('第一轮')
    expect(useChatStore.getState().messages.at(-1)?.content).toBe(FIRST)
    expect(useChatStore.getState().busy).toBe(false)

    // out-of-band straggler for the round that already ended: it must be
    // rejected outright (the stream was really closed), i.e. no buffer growth
    gw.straggler(SECOND)
    expect(useChatStore.getState().streamRaw).toBe('')
    expect(useChatStore.getState().messages.at(-1)?.content).toBe(FIRST)

    // and the next round starts from a clean buffer with none of it inside
    await send('第二轮')
    const last = useChatStore.getState().messages.at(-1)?.content ?? ''
    expect(last).toBe('C')
